...
import { UserPlus, UserCheck, MessageSquare, UserMinus, Clock, Mail, Building2, Search } from "lucide-react";
...
const RecruiterConnections = () => {
  ...
  const searchUsers = async (query: string) => {
    ...
    try {
      console.log(`Searching for recruiters: ${query}`);
      ...
      if (res.ok) {
        ...
        // Filter to show recruiters only
        const recruiters = (data.users || []).filter((u: any) => u.role === "recruiter");
        console.log(`Search returned ${recruiters.length} recruiter results`);
  ...
  const handleAcceptRequestFromSearch = async (requestId: string) => {
    const token = localStorage.getItem("token");
    if (!token) return;
  ...
  const messageUser = async (userId: string, userName: string) => {
    console.log("[CONVERSATION] Starting conversation with recruiter:", userId, userName);
    const token = localStorage.getItem("token");
    if (!token) {
  ...
  const RecruiterCard = ({ user, actionButton, secondaryButton }: any) => (
    <motion.div
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-muted-foreground mt-1">Connect with other recruiters</p>
            </div>
          </div>
        </motion.div>
        ...
        <Tabs defaultValue="discover" className="w-full">
          <TabsList className="grid w-full max-w-2xl grid-cols-4">
            ...
            <TabsTrigger value="friends" className="relative">
              Connections
              {friends.length > 0 && (
                <span className="ml-2 text-xs bg-recruiter text-recruiter-foreground rounded-full px-2">
                  {friends.length}
                </span>
              )}
            </TabsTrigger>
          ...
          <TabsContent value="discover" className="space-y-4 mt-6">
            <div className="mb-8">
              <p className="text-sm text-muted-foreground mb-3">Search by name, email, or username to find other recruiters and send connection requests</p>
              <div className="relative max-w-2xl">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground w-5 h-5" />
                <Input
                  placeholder="Search for recruiters (name, email, or username)..."
                  value={searchQuery}
                  onChange={(e) => {
                    setSearchQuery(e.target.value);
                    searchUsers(e.target.value);
                  }}
                  className="pl-10 h-12 text-base rounded-lg border-2 border-muted-foreground/20 focus:border-recruiter transition-colors"
                  autoComplete="off"
                />
              </div>
              {searchQuery && (
            ...
            {searching ? (
              <div className="text-center py-12">
                <div className="inline-flex items-center gap-2">
                  ...
                  >
                    <Search className="w-5 h-5 text-recruiter" />
                  </motion.div>
              ...
              <div className="text-center py-16">
                ...
                <p className="text-lg font-medium text-muted-foreground">Start Discovering Recruiters</p>
                <p className="text-sm text-muted-foreground mt-1">Search by name, email, or username to find and connect with other recruiters</p>
              </div>
                    ...
                    <RecruiterCard
                      ...
                      actionButton={
                        user.connection_status === "friends" ? (
                          ...
                          <Button
                            className="flex-1 bg-recruiter"
                            onClick={() => handleAcceptRequestFromSearch(user.request_id)}
                          >
                          ...
                          <Button
                            className="flex-1 bg-recruiter hover:bg-recruiter/90"
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  {friends.length === 0 ? "No connections yet. Start by sending connection requests!" : "No matches"}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
